...
export async function POST(req) {
  try {
    ...
    const { id_dossier,tribunal, autre,montant, user, numero, date_audience, commentaire } = body;
    ...
    if(dossier.type==2){
      let doss = await executeQuery('select * from client where id=? ',[dossier.defendeur])
      def=doss[0].name
    }
    ...
    const [dossier_row,result] = await Promise.all([
      executeQuery(
        'INSERT INTO dossiers (uuid, type, id_grand, numero, demandeur, defendeur, precedent, tribunal,avocat_ancien,avocat_new, comment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [
              newUuid,
              dossier.type,
              dossier.id_grand,
        ...
        executeQuery("UPDATE dossiers SET appel = 1 WHERE id = ?",[id_dossier])
    ]);
  ...
  if(montant!=0){
    await executeQuery('INSERT INTO caisse (dossier, montant,user) VALUES (?, ?,?)',[dossier_id, montant,user]);
  }
