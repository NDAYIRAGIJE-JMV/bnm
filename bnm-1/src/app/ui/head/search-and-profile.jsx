  ...
  return (
    <div className="h-[3.5rem]  md:h-[4.5rem] xl:h-[3.5rem] w-full">
      <div className="h-full w-[100%] md:w-[95%] flex justify-between mt-2 md:mt-1 md:items-center mx-auto">
        ...
        <div className="relative h-[3rem] w-[3rem] sm:h-[3.5rem] sm:w-[3.5rem] md:h-[4.2rem] md:w-[4.2rem] xl:h-[2.8rem] xl:w-[2.8rem] rounded-full">
          <Image title={session && session!='unlogged' && session.name}
        ...
        {session && session!='unlogged' && (<span className='absolute right-8 top-16 text-white'>{session.name}</span>)}
      </div>
    </div>
  );
}
