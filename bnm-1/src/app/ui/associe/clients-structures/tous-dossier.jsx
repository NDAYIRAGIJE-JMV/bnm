...
const TousDossiers = ({closeModal, id, name}) => {
  ...
  const fetchData = async () => {
    ...
    if (id) {
      try {
        ...
        const res = await fetch('/secretaire/api/clients-structures/tousdossiers', requestOptions);
  ...
  return (
    <div className="w-full h-full flex justify-center items-center absolute bg-slate-700 bg-opacity-25 z-25">
      <div className="w-[95%] md:w-[95%] lg:w-[93%] xl:w-[95%] h-[95%] md:h-[97%] xl:h-[98%]  relative flex flex-col space-y-3 md:space-y-2 bg-white rounded-md mx-auto rounded-md">
        <div className="w-[98%] h-[5%] flex justify-between items-center mx-auto mt-2">
        <div><span>Tous les dossiers de <span className='font-bold'>{name}</span> </span></div>
          <button
            onClick={closeModal}
